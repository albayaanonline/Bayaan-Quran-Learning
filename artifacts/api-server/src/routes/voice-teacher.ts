...
router.post("/voice-teacher/message", requireAuth, async (req: any, res) => {
  ...
  try {
    ...
    if (audioBase64 && audioBase64.length > 100) {
      ...
      if (result.success && result.text.trim()) {
        ...
        if (!text) {
          ...
          res.write(`data: ${JSON.stringify({ content: "I couldn't hear you clearly. Could you try again? Make sure your microphone is working and speak clearly." })}\n\n`);
    ...
    if (!userText.trim()) {
      setSSEHeaders(res);
      res.write(`data: ${JSON.stringify({ transcribedText: "" })}\n\n`);
      res.write(`data: ${JSON.stringify({ content: "I didn't catch that. Could you please repeat?" })}\n\n`);
      res.write(`data: ${JSON.stringify({ done: true })}\n\n`);
      res.end();
      return;
    }
