...
export default function Dashboard() {
  ...
  if (!dashboard) return <AppLayout><div>Failed to load dashboard.</div></AppLayout>;
      ...
      <div className="space-y-8 max-w-6xl mx-auto">
        <div>
          <h1 className="text-3xl font-serif font-bold tracking-tight text-emerald-950 dark:text-emerald-50">
            Welcome back, {dashboard.profile.displayName}
          </h1>
              ...
              <CardContent className="p-6 flex flex-col justify-center h-full">
                ...
                <p className="text-sm text-muted-foreground mt-4">
                  {dashboard.todayMinutes >= dashboard.streak.dailyGoalMinutes 
                    ? "Goal reached! Mashallah, keep going." 
                    : `${dashboard.streak.dailyGoalMinutes - dashboard.todayMinutes} minutes left to reach your daily goal.`}
                </p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          >
            <Card className="h-full border-orange-200 bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950/40 dark:to-amber-950/40">
              <CardContent className="p-6 flex flex-col items-center justify-center text-center h-full">
                ...
                <p className="text-orange-800/80 dark:text-orange-200/80 font-medium">Day Streak</p>
              </CardContent>
            </Card>
          </motion.div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-4">
          {[
            { title: "Total XP", value: dashboard.streak.xp, icon: Star, color: "text-blue-600", bg: "bg-blue-100 dark:bg-blue-900/50" },
            { title: "Ayahs Read", value: dashboard.totalAyahs, icon: BookOpen, color: "text-emerald-600", bg: "bg-emerald-100 dark:bg-emerald-900/50" },
            { title: "Avg Accuracy", value: `${dashboard.quickStats.avgAccuracy}%`, icon: Target, color: "text-purple-600", bg: "bg-purple-100 dark:bg-purple-900/50" },
            { title: "Total Time", value: `${Math.round(dashboard.quickStats.totalRecordings / 60)}h`, icon: Clock, color: "text-rose-600", bg: "bg-rose-100 dark:bg-rose-900/50" },
          ].map((stat, i) => (
            <motion.div
              key={stat.title}
              initial={{ opacity: 0, y: 20 }}
