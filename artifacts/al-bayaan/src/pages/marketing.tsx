    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-950 via-emerald-900 to-emerald-800 text-white">
      ...
      <section className="max-w-5xl mx-auto px-4 pt-20 pb-16 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Badge className="bg-emerald-700 text-emerald-100 border-emerald-600 mb-6">🕌 World-class Islamic Education</Badge>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/sign-up">
              <Button size="lg" className="bg-emerald-400 hover:bg-emerald-300 text-emerald-950 font-bold px-8">
                Start Learning Free <ArrowRight className="h-5 w-5 ml-2" />
              </Button>
            </Link>
            <Link href="/sign-in">
              <Button size="lg" variant="outline" className="border-emerald-400 text-emerald-100 hover:bg-emerald-800">
                Sign In
              </Button>
            </Link>
          </div>
          <p className="text-sm text-emerald-400 mt-4">No credit card required • Free forever plan • Cancel anytime</p>
        </motion.div>
      </section>

      {/* Stats */}
      <section className="bg-emerald-800/50 border-y border-emerald-700">
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        ...
        <p className="text-emerald-300 text-center mb-10">A complete Islamic learning ecosystem</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
              <Card className="bg-emerald-800/60 border-emerald-700 hover:bg-emerald-800/80 transition-colors h-full">
                <CardContent className="p-5">
                  <div className="h-10 w-10 rounded-xl bg-emerald-700 flex items-center justify-center mb-3">
      ...
      {/* Testimonials */}
      <section className="bg-emerald-900/60 border-y border-emerald-700">
        <div className="max-w-5xl mx-auto px-4 py-16">
          <h2 className="text-3xl font-serif font-bold text-center mb-10">What Students Say</h2>
          <div className="grid sm:grid-cols-3 gap-5">
            {TESTIMONIALS.map((t, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                <Card className="bg-emerald-800/50 border-emerald-700 h-full">
                  <CardContent className="p-5 flex flex-col h-full">
                    <StarRating count={t.stars} />
                    <p className="text-emerald-100 text-sm mt-3 flex-1 italic">"{t.quote}"</p>
      ...
      <section className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-emerald-300 mb-6">Get Islamic learning tips, Quran reminders, and platform updates.</p>
        {submitted ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-center gap-2 text-emerald-300">
          ...
          <form onSubmit={handleSubmit} className="flex gap-2 max-w-md mx-auto">
            <Input
              ...
              className="bg-emerald-800/50 border-emerald-600 text-white placeholder:text-emerald-400"
            />
            <Button type="submit" disabled={loading} className="bg-emerald-400 text-emerald-950 font-bold hover:bg-emerald-300 shrink-0">
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Subscribe"}
            </Button>
          </form>
        )}
        <p className="text-xs text-emerald-500 mt-3">No spam. Unsubscribe anytime.</p>
      </section>

      {/* CTA */}
      <section className="bg-emerald-700 text-center py-12 px-4">
        <h2 className="text-2xl font-serif font-bold mb-3">Begin Your Journey Today</h2>
        <p className="text-emerald-200 mb-6">بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيم</p>
        <Link href="/sign-up">
          <Button size="lg" className="bg-white text-emerald-900 font-bold hover:bg-emerald-100 px-8">
            Start Free — No Account Needed for Demo <ArrowRight className="h-4 w-4 ml-2" />
          </Button>
        </Link>
      </section>

      <footer className="bg-emerald-950 text-center py-8 text-emerald-500 text-sm">
