...
const TYPE_COLORS: Record<string, string> = {
  exam_passed: "text-blue-700 bg-blue-100",
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-blue-100 z-50 overflow-hidden"
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-blue-50">
              <div className="flex items-center gap-2">
                <Bell className="h-4 w-4 text-blue-700" />
                <span className="font-semibold text-sm text-slate-900">{t("notif.title")}</span>
                {unread > 0 && (
                  <Badge className="h-4 text-[10px] px-1.5 bg-red-500 text-white border-0">{unread}</Badge>
                )}
              </div>
              {unread > 0 && (
                <button onClick={markAllRead} className="text-[11px] text-blue-700 hover:text-blue-800 flex items-center gap-1">
            ...
            <ScrollArea className="max-h-80">
              {notifications.length === 0 ? (
                <div className="py-10 text-center">
                  <Bell className="h-8 w-8 text-blue-200 mx-auto mb-2" />
                ...
                <div className="divide-y divide-blue-50">
                  {notifications.map(notif => {
                    ...
                    return (
                      <div
                        key={notif.id}
                        className={`flex gap-3 px-4 py-3 hover:bg-blue-50/50 cursor-pointer transition-colors ${!notif.isRead ? "bg-blue-50/30" : ""}`}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-1">
                            <p className={`text-xs font-semibold truncate ${!notif.isRead ? "text-slate-900" : "text-foreground"}`}>{notif.title}</p>
